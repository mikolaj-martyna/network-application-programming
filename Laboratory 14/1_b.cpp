...
int verifyCertificate(const int preverify_ok, const X509_STORE_CTX* ctx) {
    if (!preverify_ok) {
...
int main() {
    ...
    SSL_set_verify(ssl, SSL_VERIFY_PEER, verifyCertificate);
    ...
    char serverName[256];
    X509_NAME_oneline(X509_get_subject_name(cert), serverName, sizeof(serverName));
    cout << "Server certificate:" << endl;
    cout << "  Subject: " << serverName << endl;

    char issuerName[256];
    X509_NAME_oneline(X509_get_issuer_name(cert), issuerName, sizeof(issuerName));
    cout << "  Issuer: " << issuerName << endl;
