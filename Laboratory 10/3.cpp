...
string decodeWebSocketFrame(const uint8_t *buffer, size_t length) {
    ...
    if (const uint8_t opcode = buffer[0] & 0x0F; opcode != 1) return "";
    ...
    if (masked) {
        ...
        for (int i = 0; i < 4; ++i) {
            maskingKey[i] = buffer[payloadOffset + i];
        }

        payloadOffset += 4;
    }

    if (payloadOffset + actualPayloadLen > length) {
        return "";
    }

    string payload;
    payload.reserve(actualPayloadLen);

    for (size_t i = 0; i < actualPayloadLen; ++i) {
        uint8_t byte = buffer[payloadOffset + i];
...
int main() {
    ...
    const string message = "Lorem ipsum dolor sit amet, consectetur adipiscing elit. Donec ante purus,"
            "commodo eu commodo vel, condimentum ac turpis. In ac odio nunc. Nullam imperdiet"
            "sapien ipsum, quis semper ante lacinia at. Aliquam a ex egestas, pharetra ligula"
            "hendrerit, laoreet magna. Proin rutrum euismod maximus. Sed turpis lacus, viverra"
            "vitae augue eu, euismod varius purus. Ut nisi ex, facilisis id odio in, molestie"
            "posuere turpis. Aenean feugiat dolor diam, vel aliquam nibh faucibus et. ";

    cout << "Message length: " << message.length() << " bytes" << endl;
    ...
    if (const string response(buffer, bytesReceived); response.find("HTTP/1.1 101") != string::npos &&
        ...
        cout << "Sent message of length " << message.length() << " bytes" << endl;
