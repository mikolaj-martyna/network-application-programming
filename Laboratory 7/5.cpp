...
int main() {
    ...
    cout << "Server response to LIST command:" << endl;
    cout << buffer << endl;
    ...
    while ((pos = response.find("\r\n")) != string::npos) {
        ...
        int msgNum = 0, msgSize = 0;
        sscanf(line.c_str(), "%d %d", &msgNum, &msgSize);

        if (msgNum > 0 && msgSize > 0) {
    ...
    auto [index, size] = *ranges::min_element(messageSizes,
                                              [](const pair<int, int>& a, const pair<int, int>& b) {
                                                  return a.second < b.second;
                                              });

    cout << "Smallest message is #" << index << " with size " << size << " bytes." << endl;

    string deleCmd = "DELE " + to_string(index) + "\r\n";
    if (send(clientSocket, deleCmd.c_str(), deleCmd.length(), 0) < 0) {
        cerr << "Failed to send DELE command." << endl;
    ...
    memset(buffer, 0, sizeof(buffer));
    if (recv(clientSocket, buffer, sizeof(buffer) - 1, 0) < 0) {
        cerr << "Failed to receive response to DELE command." << endl;
    ...
    if (strncmp(buffer, "+OK", 3) == 0) {
        cout << "Marked message #" << index << " for deletion." << endl;
    } else {
        cout << "Failed to delete message #" << index << endl;
    }
    ...
    cout << "Server: " << buffer;
    cout << "Session closed. The marked message has been permanently deleted." << endl;
