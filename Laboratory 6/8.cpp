...
string getFileExtension(const string &filename) {
    if (const size_t dotPos = filename.rfind('.'); dotPos != string::npos) {
...
string getFilename(const string &path) {
    if (const size_t lastSlash = path.find_last_of("/\\"); lastSlash != string::npos) {
...
int main() {
    ...
    const string attachmentPath = getInput("Enter image file path: ");

    const string attachmentName = getFilename(attachmentPath);
    const string fileExtension = getFileExtension(attachmentName);
    const string mimeType = getMimeType(fileExtension);

    if (mimeType.find("image/") == string::npos && mimeType != "application/octet-stream") {
        cerr << "The file does not appear to be an image file." << endl;
    ...
    if (base64Attachment.empty()) {
        cerr << "Failed to read image file. Aborting." << endl;
    ...
    message += "--" + boundary + "\r\n";
    message += "Content-Type: " + mimeType + "; name=\"" + attachmentName + "\"\r\n";
    ...
    cout << "Email with image attachment sent successfully to " << recipients.size() << " recipient(s)!" << endl;
